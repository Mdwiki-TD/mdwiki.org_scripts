...
class TestDup:
    def test_get_renders_form(self, mock_client):
        r = mock_client.get("/dup/")
    ...
    def test_post_starts_job_and_redirects(self, mock_client, login, csrf_token, monkeypatch):
        ...
        login("Doc James")
        r = mock_client.post("/dup/", data={"start": "start", "csrf_token": csrf_token("/dup/")})
    ...
    def test_concurrent_post_returns_existing_job(self, mock_client, login, csrf_token, monkeypatch):
        ...
        login("Doc James")
        r1 = mock_client.post("/dup/", data={"start": "start", "csrf_token": csrf_token("/dup/")})
        r2 = mock_client.post("/dup/", data={"start": "start", "csrf_token": csrf_token("/dup/")})
