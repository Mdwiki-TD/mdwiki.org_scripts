...
def test_create_job():
    ...
    job = JobsService().create_job("collect_templates_data", username="test_user")
...
def test_get_job():
    ...
    created_job = JobsService().create_job("collect_templates_data", username="z")

    retrieved_job = JobsService().get_job(created_job.id, job_type="collect_templates_data")
...
def test_get_nonexistent_job():
    ...
    with pytest.raises(LookupError, match="Job id 999 was not found"):
        JobsService().get_job(999, job_type="collect_templates_data")


def test_list_jobs():
    """Test listing jobs."""
    job1 = JobsService().create_job("collect_templates_data", username="z")
    JobsService().update_job_status(job1.id, "completed", job_type="collect_templates_data")
    JobsService().create_job("collect_templates_data", username="z")
    JobsService().create_job("other_job", username="z")
...
def test_list_jobs_with_limit():
    ...
    for i in range(5):
        ...
        if i < 4:
            JobsService().update_job_status(job.id, "completed", job_type="collect_templates_data")

    jobs = JobsService().list_jobs(limit=2)
...
def test_update_job_status():
    ...
    updated_job = JobsService().update_job_status(job.id, "running", job_type="collect_templates_data")
...
def test_list_jobs_filtered_by_type():
    ...
    JobsService().update_job_status(job1.id, "completed", job_type="collect_templates_data")
    JobsService().create_job("collect_templates_data", username="z")
    JobsService().create_job("fix_nested_main_files", username="z")
    JobsService().create_job("other_job_type", username="z")

    collect_jobs = JobsService().list_jobs(job_type="collect_templates_data")
    assert len(collect_jobs) == 2
    assert all(job.job_type == "collect_templates_data" for job in collect_jobs)

    fix_jobs = JobsService().list_jobs(job_type="fix_nested_main_files")
    assert len(fix_jobs) == 1
    assert all(job.job_type == "fix_nested_main_files" for job in fix_jobs)

    all_jobs = JobsService().list_jobs()
...
def test_list_jobs_filtered_with_limit():
    ...
    for i in range(5):
        ...
        if i < 4:
            JobsService().update_job_status(job.id, "completed", job_type="collect_templates_data")
    for i in range(3):
        job = JobsService().create_job("fix_nested_main_files", username="z")
        if i < 2:
            JobsService().update_job_status(job.id, "completed", job_type="fix_nested_main_files")

    collect_jobs = JobsService().list_jobs(limit=2, job_type="collect_templates_data")
    assert len(collect_jobs) == 2
    assert all(job.job_type == "collect_templates_data" for job in collect_jobs)
...
def test_update_job_status_nonexistent():
    ...
    with pytest.raises(LookupError):
        JobsService().update_job_status(999, "completed", job_type="test_job")
...
class TestIsJobCancelled:
    ...
    def test_cancelled_status_returns_true(self, monkeypatch):
        ...
        result = JobsService().is_job_cancelled(1, "test")
    ...
    def test_refresh_called_before_checking_status(self, monkeypatch):
        ...
        JobsService().is_job_cancelled(1, "test")
...
class TestGetJob:
    ...
    def test_returns_job_when_found(self):
        job = JobsService().create_job("test_job", username="test_user")
        result = JobsService().get_job(job.id, "test_job")
        assert result.id == job.id
        assert result.job_type == "test_job"

    def test_raises_lookup_error_when_not_found(self):
        with pytest.raises(LookupError, match="Job id 999 was not found"):
            JobsService().get_job(999, "test_job")
...
class TestGetAllUserJobsStats:
    ...
    def test_returns_stats_with_correct_counts(self, monkeypatch):
        ...
        result = JobsService().get_all_user_jobs_stats("test_user")
    ...
    def test_respects_limit_parameter(self, monkeypatch):
        ...
        monkeypatch.setattr("src.main_app.db.services.jobs_service.db.session.query", query_side_effect)
        result = JobsService().get_all_user_jobs_stats("test_user", limit=5)
...
class TestGetUserJobsStats:
    ...
    def test_with_jobs_types_filters_correctly(self, monkeypatch):
        ...
        monkeypatch.setattr("src.main_app.db.services.jobs_service.db.session.query", query_side_effect)
        result = JobsService().get_user_jobs_stats("test_user", jobs_types=["type_a", "type_b"])
...
class TestHasActiveJob:
    ...
    def test_returns_true_when_active_job_exists(self):
        JobsService().create_job("test_job", username="test_user")
        assert JobsService().has_active_job("test_job") is True

    def test_returns_false_when_no_active_job(self):
        job = JobsService().create_job("test_job", username="test_user")
        JobsService().update_job_status(job.id, "completed", job_type="test_job")
        assert JobsService().has_active_job("test_job") is False
...
class TestCancelJobDb:
    ...
    def test_cancels_pending_job(self):
        job = JobsService().create_job("test_job", username="test_user")
        result = JobsService().cancel_job_db(job.id)
        assert result is True
        cancelled = JobsService().get_job(job.id, "test_job")
        assert cancelled.status == "cancelled"

    def test_cancels_running_job(self):
        job = JobsService().create_job("test_job", username="test_user")
        JobsService().update_job_status(job.id, "running", job_type="test_job")
        result = JobsService().cancel_job_db(job.id)
        assert result is True
        cancelled = JobsService().get_job(job.id, "test_job")
        assert cancelled.status == "cancelled"
    ...
    def test_with_job_type_filter_works(self):
        job1 = JobsService().create_job("type_a", username="test_user")
        JobsService().create_job("type_b", username="test_user")
        result = JobsService().cancel_job_db(job1.id, job_type="type_a")
        assert result is True
        cancelled = JobsService().get_job(job1.id, "type_a")
        assert cancelled.status == "cancelled"
...
class TestUpdateJobStatus:
    ...
    def test_sets_started_at_when_running_and_not_previously_set(self):
        ...
        updated = JobsService().update_job_status(job.id, "running", job_type="test_job")
        assert updated.status == "running"
        assert updated.started_at is not None

    def test_sets_completed_at_for_final_statuses(self):
        job = JobsService().create_job("test_job", username="test_user")
        updated = JobsService().update_job_status(job.id, "completed", job_type="test_job")
        assert updated.status == "completed"
        assert updated.completed_at is not None

    def test_sets_completed_at_for_cancelled(self):
        job = JobsService().create_job("test_job", username="test_user")
        updated = JobsService().update_job_status(job.id, "cancelled", job_type="test_job")
    ...
    def test_re_raises_after_max_retries(self, monkeypatch):
        ...
        with pytest.raises(OperationalError):
            JobsService().update_job_status(1, "completed", job_type="test_job")
...
class TestUpdateJobStatusWithRetry:
    ...
    def test_retries_on_connection_error(self, monkeypatch):
        ...
        result = JobsService().update_job_status_with_retry(1, "completed", job_type="test_job")
