...
def test_jobs_service_lifecycle(mock_app: Flask) -> None:
...
def test_get_job_not_found(mock_app: Flask) -> None:
    with mock_app.app_context():
        with pytest.raises(LookupError):
            get_job(999, "any")


def test_update_status_not_found(mock_app: Flask) -> None:
    with mock_app.app_context():
        with pytest.raises(LookupError):
            update_job_status(999, "running", job_type="any")


def test_cancel_non_existent(mock_app: Flask) -> None:
    with mock_app.app_context():
        assert cancel_job_db(999, "any") is False


def test_delete_non_existent(mock_app: Flask) -> None:
    with mock_app.app_context():
...
def test_create_job_after_completed_succeeds(mock_app: Flask) -> None:
...
def test_create_job_after_cancelled_succeeds(mock_app: Flask) -> None:
...
def test_create_different_type_while_active_succeeds(mock_app: Flask) -> None:
