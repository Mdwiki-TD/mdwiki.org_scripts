...
def _replace_links(text: str, oldlink: str, newlink: str, state: _RunState) -> str:
...
def _treat_page(api: AllAPIS, title: str, state: _RunState, *, save: bool) -> str:
    ...
    for tt, info in links["links"].items():
        tit = info["title"]
        tit2 = state.normalized.get(tit, tit)
        target = state.from_to.get(tit) or state.from_to.get(tit2)
        if target:
            newtext = _replace_links(newtext, tit, target, state)
...
def run(
    *,
    ...
    ``title`` is either a specific page name or the literal ``"all"`` (the
    default), which iterates every mainspace non-redirect page on mdwiki.
    """
